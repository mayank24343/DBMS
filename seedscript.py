...
if not settings.configured:
    settings.configure(
        SECRET_KEY='a-dummy-secret-key-for-standalone-script',
...
for i in range(10):
    ...
    for j in range(1,6):
        ...
        for k in range(6):
            wid += 1
            f.write(f"INSERT INTO users(userid,password_hash,role) VALUES ({wid},'{DEFAULT_HASH}','worker');\n")
...
for i in range(1,1001):
    index = wid+i
    f.write(f"INSERT INTO users(userid,password_hash,role) VALUES ({index},'{DEFAULT_HASH}','citizen');\n")
