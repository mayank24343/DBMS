...
for i,(t,d) in enumerate(vaccines,1):
    ...
    f.write(f"INSERT INTO vaccine_prereq_age VALUES (default,{20+i},{random.randint(0,10)})")
...
for i in range(1,41):
    index = i 
    f.write(f"INSERT INTO users(userid,password,role) VALUES ({index},'password','supplier');\n")
...
for i in range(10):
    ...
    for j in range(1,6):
        ...
        for k in range(6):
            wid += 1
            f.write(f"INSERT INTO users(userid,password,role) VALUES ({wid},'password','worker');\n")
            f.write(f"""INSERT INTO healthcareworker VALUES({wid},'Worker{wid}','Doctor');""")
            f.write(f"""INSERT INTO works VALUES({wid},{i*10+j});""")
...
for i in range(10,20):
    ...
    for j in range(1,6):
        ...
        for k,(t,x,d) in enumerate(medicines,1):
            f.write(f"INSERT INTO inventory VALUES (default,{i*10+j},{k},{random.randint(20,50)*100},'{rand_date(datetime(2026,1,1),datetime(2028,1,1)).date()}',default);\n")
        for k,(t,d) in enumerate(vaccines,1):
            f.write(f"INSERT INTO inventory VALUES (default,{i*10+j},{20+k},{random.randint(20,50)*100},'{rand_date(datetime(2026,1,1),datetime(2028,1,1)).date()}',default);\n")
        for k in range(1,11):
            f.write(f"INSERT INTO inventory VALUES (default,{i*10+j},{30+k},{random.randint(20,50)*100},'9999-12-31',default);\n")        
...
for i in range(1,1001):
    index = wid+i
    f.write(f"INSERT INTO users(userid,password,role) VALUES ({index},'password','citizen');\n")
...
""")
    ...
    if dob.year <= 2010:
        for k in range(21,31):
            if random.randint(1,10) >= 5:
                ...
                f.write(f"""INSERT INTO vaccination VALUES ({i},{index},{k},'{date}',{1},{random.choice(vacc_places)});""")
    ...
    if sex == 'F' and dob.year <= 2000 and random.randint(1,10) < 4:
        ...
        for k in range(random.randint(0,2)):
            ...
            for p in range(random.randint(1,2)):
                f.write(f"INSERT INTO doctor_visit VALUES({vid}, {index}, {random.choice(docs[centre])});")
    ...
    if random.randint(1,100) <= 5:
        ...
        if random.randint(1,10) >= 5 and flag1:
            ...
            for p in [1,3,4]:
                f.write(f"INSERT INTO prescription VALUES(default, {vid}, {p},'1 tablet','2/day','{date}','{(datetime(2026,3,18) + timedelta(days=random.randint(10,12))).date()}','After food');")
            ...
            f.write(f"INSERT INTO diagnosis VALUES(default, {vid}, {6}, '{diseases[5][0]}');")
            for p in [1,3,4]:
                f.write(f"INSERT INTO prescription VALUES(default, {vid}, {p},'1 tablet','2/day','{date.date()}','{(date + timedelta(days=random.randint(10,12))).date()}','After food');")
    ...
    for disease_id, name, meds, labss in [ (12, "COVID", [1,20], [16,17]),(16, "Dengue", [1], [18]),(7, "Malaria", [14], [19,20])]:
        if random.random() < 0.5:
            ...
            for m in meds:
                f.write(
                f"INSERT INTO prescription VALUES(default,{vid},{m},'1 tablet','2/day','{date.date()}','{(date + timedelta(days=5)).date()}','After food');"
                )
