...
import { searchHealthDirectory } from '../services/api';
import { Search, FlaskConical, Pill, Activity, MapPin, Phone } from 'lucide-react';
...
const ServiceDirectory = () => {
    ...
    const handleSearch = async (e) => {
        ...
        setIsSearching(true);
        const data = await searchHealthDirectory(searchType, searchQuery);
        setResults(data);
        setHasSearched(true);
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-10">
                ...
                <div className="flex border-b border-gray-200 bg-gray-50">
                    ...
                    <button 
                        ...
                        className={`flex-1 py-4 text-sm font-bold flex justify-center items-center gap-2 transition-colors ${searchType === 'pharmacy' ? 'bg-white text-emerald-700 border-b-2 border-emerald-700' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                            ...
                            {results.map((facility) => (
                                <div key={facility.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                                    ...
                                    <div className="space-y-2 text-sm text-gray-600 mb-6">
                                        <p className="flex items-center gap-2"><MapPin className="w-4 h-4 text-gray-400" /> {facility.address}</p>
                                        <p className="flex items-center gap-2"><Phone className="w-4 h-4 text-gray-400" /> {facility.contact}</p>
                                    </div>

                                    {/* Action Button */}
                                    <Link 
                                        to={`/citizen/123456789012/book`} // Using the hardcoded citizen ID for demo purposes
